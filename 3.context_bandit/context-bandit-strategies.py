...
class MultiArmedBandit:
    def __init__(self, arms, grid_size=50, strategy='epsilon_greedy', epsilon=0.1, max_speed=0.1, start_x=0.5, start_y=0.5, alpha=1.0):
        self.arms = arms
        ...
        self.alpha = alpha  # Exploration parameter for LinUCB
        self.d = 3  # Dimension of the context (time, x, y)
        self.A = [np.eye(self.d) for _ in range(self.n_arms)]
        self.b = [np.zeros((self.d, 1)) for _ in range(self.n_arms)]
        self.theta = [np.zeros((self.d, 1)) for _ in range(self.n_arms)]
    ...
    def select_action_linucb(self, reward_grid):
        context = np.array([[self.t], [self.x], [self.y]])
        ucb_values = np.zeros(self.n_arms)
        
        for i in range(self.n_arms):
            theta = np.linalg.inv(self.A[i]) @ self.b[i]
            p = theta.T @ context + self.alpha * np.sqrt(context.T @ np.linalg.inv(self.A[i]) @ context)
            ucb_values[i] = p.item()
        
        best_arm = np.argmax(ucb_values)
        
        # Use gradient ascent to find the best path, similar to greedy strategy
        # The reward_grid is already the combined reward grid
        grad_y, grad_x = np.gradient(reward_grid)
        start_x, start_y = int(self.x * (self.grid_size - 1)), int(self.y * (self.grid_size - 1))
    ...
    def select_action(self, reward_grid):
        if self.strategy == 'random':
            ...
            return self.select_action_linucb(reward_grid)
        else:
    ...
    def update_linucb(self, arm_index, context, reward):
        self.A[arm_index] += context @ context.T
        self.b[arm_index] += reward * context
        self.theta[arm_index] = np.linalg.inv(self.A[arm_index]) @ self.b[arm_index]
...
def run_simulation(mab, n_steps):
    ...
    for step in range(n_steps):
        ...
        # Update LinUCB parameters if using LinUCB strategy
        if mab.strategy == 'linucb':
            context = np.array([[mab.t], [end_x], [end_y]])
            mab.update_linucb(arm_id, context, reward)
